...
> Week Review может выполняться как до, так и после session-prep — порядок зависит от локального расписания.
> Если секция «Итоги W{N-1}» уже существует, перенеси её данные без пересчёта. Если её ещё нет, НЕ собирай коммиты и НЕ вычисляй X/Y самостоятельно: оставь явный placeholder для последующего Week Review.
...
> Если секция итогов не найдена — это штатно при расписании, где session-prep идёт раньше Week Review. В новом WeekPlan запиши `**Статус итогов:** ожидает Week Review`; carry-over можно предварительно собрать из WP context files, но completion rate X/Y не указывать. Week Review позже заменит placeholder авторитетными итогами.
...
**Статус итогов:** ожидает Week Review

**Предварительный carry-over:**
- #N — что осталось

> Week Review заменит этот блок итоговыми метриками, таблицей РП, инсайтами и carry-over. Не подставляй X/Y до его запуска.
